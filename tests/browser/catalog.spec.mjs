...
test("records the catalog browser baseline", async ({ page }, testInfo) => {
  ...
  const baseline = await page.locator(".slide").evaluateAll((slides) => ({
    ...
    markerCount: slides.filter(
      (slide) =>
        slide.querySelectorAll(":scope > .synthetic-marker").length === 1 &&
        slide
          .querySelector(":scope > .synthetic-marker")
          ?.textContent?.includes("NOT FOR CLINICAL OR ACADEMIC USE"),
    ).length,
    overflowCount: slides.filter(
      (slide) =>
        slide.scrollWidth > slide.clientWidth + 1 ||
        slide.scrollHeight > slide.clientHeight + 1,
    ).length,
  }));
  const chartStatus = await page
    .locator(".chart-host")
    .evaluateAll((charts) => ({
      count: charts.length,
      ready: charts.filter(
        (chart) =>
          chart.dataset.chartReady === "true" && chart.querySelector("svg"),
      ).length,
    }));
  ...
  const kmSeries = await page
    .locator("[data-line-style]")
    .evaluateAll((nodes) =>
      nodes.map((node) => ({
  ...
  // Legend uses short group labels; dash style is asserted via data-line-style above.
  const kmLabels = await page
    .locator("#chartKM text.chart-label")
    .allTextContents();
  expect(kmLabels).toEqual(
    expect.arrayContaining(["Group A", "Group B", "Number at Risk"]),
  );
